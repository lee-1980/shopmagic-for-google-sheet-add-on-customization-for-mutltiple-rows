...
trait ServiceAccountSignerTrait
{
    ...
    public function signBlob($stringToSign, $forceOpenssl = \false)
    {
        ...
        $signedString = '';
        if (\class_exists('ShopMagicGoogleSheetsVendor\\phpseclib\\Crypt\\RSA') && !$forceOpenssl) {
            $rsa = new \ShopMagicGoogleSheetsVendor\phpseclib\Crypt\RSA();
            $rsa->loadKey($privateKey);
            $rsa->setSignatureMode(\ShopMagicGoogleSheetsVendor\phpseclib\Crypt\RSA::SIGNATURE_PKCS1);
            $rsa->setHash('sha256');
