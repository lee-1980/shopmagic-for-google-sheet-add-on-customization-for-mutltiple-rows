...
use ShopMagicGoogleSheetsVendor\GuzzleHttp\Psr7\Query;
 ...
 */
class OAuth2 implements \ShopMagicGoogleSheetsVendor\Google\Auth\FetchAuthTokenInterface
{
    ...
    public function toJwt(array $config = [])
    {
        ...
        return \ShopMagicGoogleSheetsVendor\Firebase\JWT\JWT::encode($assertion, $this->getSigningKey(), $this->getSigningAlgorithm(), $this->getSigningKeyId());
    }
    /**
    ...
    public function generateCredentialsRequest()
    {
        ...
        return new \ShopMagicGoogleSheetsVendor\GuzzleHttp\Psr7\Request('POST', $uri, $headers, \ShopMagicGoogleSheetsVendor\GuzzleHttp\Psr7\Query::build($params));
    }
    /**
    ...
    public function fetchAuthToken(callable $httpHandler = null)
    {
        if (\is_null($httpHandler)) {
            $httpHandler = \ShopMagicGoogleSheetsVendor\Google\Auth\HttpHandler\HttpHandlerFactory::build(\ShopMagicGoogleSheetsVendor\Google\Auth\HttpHandler\HttpClientCache::getHttpClient());
        }
     ...
     * @throws \Exception
     */
    public function parseTokenResponse(\ShopMagicGoogleSheetsVendor\Psr\Http\Message\ResponseInterface $resp)
    {
    ...
    public function buildFullAuthorizationUri(array $config = [])
    {
        ...
        $existingParams = \ShopMagicGoogleSheetsVendor\GuzzleHttp\Psr7\Query::parse($result->getQuery());
        $result = $result->withQuery(\ShopMagicGoogleSheetsVendor\GuzzleHttp\Psr7\Query::build(\array_merge($existingParams, $params)));
    ...
    private function coerceUri($uri)
    {
        ...
        return \ShopMagicGoogleSheetsVendor\GuzzleHttp\Psr7\Utils::uriFor($uri);
    }
    /**
     * @param string $idToken
    ...
    private function jwtDecode($idToken, $publicKey, $allowedAlgs)
    {
        ...
        foreach ($keys as $key) {
            try {
                return \ShopMagicGoogleSheetsVendor\Firebase\JWT\JWT::decode($idToken, $key);
            } catch (\Exception $e) {
                // try next alg
            }
        }
    ...
    private function getFirebaseJwtKeys($publicKey, $allowedAlgs)
    {
        ...
        if ($publicKey instanceof \ShopMagicGoogleSheetsVendor\Firebase\JWT\Key) {
            return [$publicKey];
        }
        ...
        if (empty($allowedAlgs)) {
            ...
            foreach ((array) $publicKey as $kid => $pubKey) {
                if (!$pubKey instanceof \ShopMagicGoogleSheetsVendor\Firebase\JWT\Key) {
                    throw new \InvalidArgumentException(\sprintf('When allowed algorithms is empty, the public key must' . 'be an instance of %s or an array of %s objects', \ShopMagicGoogleSheetsVendor\Firebase\JWT\Key::class, \ShopMagicGoogleSheetsVendor\Firebase\JWT\Key::class));
                }
                $keys[$kid] = $pubKey;
            }
            return $keys;
        ...
        if (\is_array($publicKey)) {
            ...
            foreach ($publicKey as $kid => $pubKey) {
                if ($pubKey instanceof \ShopMagicGoogleSheetsVendor\Firebase\JWT\Key) {
                    ...
                    $keys[$kid] = new \ShopMagicGoogleSheetsVendor\Firebase\JWT\Key($pubKey, $allowedAlg);
                }
            }
            return $keys;
        }
        return [new \ShopMagicGoogleSheetsVendor\Firebase\JWT\Key($publicKey, $allowedAlg)];
    }
    /**
