 ...
 */
namespace ShopMagicGoogleSheetsVendor\Google\Auth;

use DateTime;
use Exception;
use ShopMagicGoogleSheetsVendor\Firebase\JWT\ExpiredException;
use ShopMagicGoogleSheetsVendor\Firebase\JWT\JWT;
use ShopMagicGoogleSheetsVendor\Firebase\JWT\Key;
use ShopMagicGoogleSheetsVendor\Firebase\JWT\SignatureInvalidException;
use ShopMagicGoogleSheetsVendor\Google\Auth\Cache\MemoryCacheItemPool;
use ShopMagicGoogleSheetsVendor\Google\Auth\HttpHandler\HttpClientCache;
use ShopMagicGoogleSheetsVendor\Google\Auth\HttpHandler\HttpHandlerFactory;
use ShopMagicGoogleSheetsVendor\GuzzleHttp\Psr7\Request;
use ShopMagicGoogleSheetsVendor\GuzzleHttp\Psr7\Utils;
use InvalidArgumentException;
use ShopMagicGoogleSheetsVendor\phpseclib\Crypt\RSA;
use ShopMagicGoogleSheetsVendor\phpseclib\Math\BigInteger as BigInteger2;
use ShopMagicGoogleSheetsVendor\phpseclib3\Crypt\PublicKeyLoader;
use ShopMagicGoogleSheetsVendor\phpseclib3\Math\BigInteger as BigInteger3;
use ShopMagicGoogleSheetsVendor\Psr\Cache\CacheItemPoolInterface;
use RuntimeException;
use ShopMagicGoogleSheetsVendor\SimpleJWT\InvalidTokenException;
use ShopMagicGoogleSheetsVendor\SimpleJWT\JWT as SimpleJWT;
use ShopMagicGoogleSheetsVendor\SimpleJWT\Keys\KeyFactory;
use ShopMagicGoogleSheetsVendor\SimpleJWT\Keys\KeySet;
...
class AccessToken
{
     ...
     */
    public function __construct(callable $httpHandler = null, \ShopMagicGoogleSheetsVendor\Psr\Cache\CacheItemPoolInterface $cache = null)
    {
        $this->httpHandler = $httpHandler ?: \ShopMagicGoogleSheetsVendor\Google\Auth\HttpHandler\HttpHandlerFactory::build(\ShopMagicGoogleSheetsVendor\Google\Auth\HttpHandler\HttpClientCache::getHttpClient());
        $this->cache = $cache ?: new \ShopMagicGoogleSheetsVendor\Google\Auth\Cache\MemoryCacheItemPool();
    }
    /**
    ...
    public function verify($token, array $options = [])
    {
        ...
        } catch (\ShopMagicGoogleSheetsVendor\Firebase\JWT\ExpiredException $e) {
            // firebase/php-jwt 5+
        } catch (\ShopMagicGoogleSheetsVendor\Firebase\JWT\SignatureInvalidException $e) {
            // firebase/php-jwt 5+
        } catch (\ShopMagicGoogleSheetsVendor\SimpleJWT\InvalidTokenException $e) {
            // simplejwt
    ...
    {
        $this->checkSimpleJwt();
        $jwkset = new \ShopMagicGoogleSheetsVendor\SimpleJWT\Keys\KeySet();
        foreach ($certs as $cert) {
            $jwkset->add(\ShopMagicGoogleSheetsVendor\SimpleJWT\Keys\KeyFactory::create($cert, 'php'));
        }
    ...
    private function verifyRs256($token, array $certs, $audience = null, $issuer = null)
    {
        ...
        foreach ($certs as $cert) {
            ...
            $keys[$cert['kid']] = new \ShopMagicGoogleSheetsVendor\Firebase\JWT\Key($publicKey, 'RS256');
        }
    ...
    public function revoke($token, array $options = [])
    {
        ...
        $body = \ShopMagicGoogleSheetsVendor\GuzzleHttp\Psr7\Utils::streamFor(\http_build_query(['token' => $token]));
        $request = new \ShopMagicGoogleSheetsVendor\GuzzleHttp\Psr7\Request('POST', self::OAUTH2_REVOKE_URI, ['Cache-Control' => 'no-store', 'Content-Type' => 'application/x-www-form-urlencoded'], $body);
    ...
    private function retrieveCertsFromLocation($url, array $options = [])
    {
        ...
        $httpHandler = $this->httpHandler;
        $response = $httpHandler(new \ShopMagicGoogleSheetsVendor\GuzzleHttp\Psr7\Request('GET', $url), $options);
    ...
    {
        if (\class_exists(\ShopMagicGoogleSheetsVendor\phpseclib\Crypt\RSA::class) && \class_exists(\ShopMagicGoogleSheetsVendor\phpseclib\Math\BigInteger::class)) {
            $key = new \ShopMagicGoogleSheetsVendor\phpseclib\Crypt\RSA();
            $key->loadKey(['n' => new \ShopMagicGoogleSheetsVendor\phpseclib\Math\BigInteger($this->callJwtStatic('urlsafeB64Decode', [$modulus]), 256), 'e' => new \ShopMagicGoogleSheetsVendor\phpseclib\Math\BigInteger($this->callJwtStatic('urlsafeB64Decode', [$exponent]), 256)]);
            return $key->getPublicKey();
        }
        $key = \ShopMagicGoogleSheetsVendor\phpseclib3\Crypt\PublicKeyLoader::load(['n' => new \ShopMagicGoogleSheetsVendor\phpseclib3\Math\BigInteger($this->callJwtStatic('urlsafeB64Decode', [$modulus]), 256), 'e' => new \ShopMagicGoogleSheetsVendor\phpseclib3\Math\BigInteger($this->callJwtStatic('urlsafeB64Decode', [$exponent]), 256)]);
    ...
    {
        if (!\class_exists('ShopMagicGoogleSheetsVendor\\phpseclib\\Crypt\\RSA')) {
            return \false;
        }
        /**
        ...
        if (\filter_var(\getenv('GAE_VM'), \FILTER_VALIDATE_BOOLEAN)) {
            if (!\defined('ShopMagicGoogleSheetsVendor\\MATH_BIGINTEGER_OPENSSL_ENABLED')) {
                \define('ShopMagicGoogleSheetsVendor\\MATH_BIGINTEGER_OPENSSL_ENABLED', \true);
            }
            if (!\defined('ShopMagicGoogleSheetsVendor\\CRYPT_RSA_MODE')) {
                \define('ShopMagicGoogleSheetsVendor\\CRYPT_RSA_MODE', \ShopMagicGoogleSheetsVendor\phpseclib\Crypt\RSA::MODE_OPENSSL);
            }
        }
        return \true;
    }
    ...
    {
        return \class_exists('ShopMagicGoogleSheetsVendor\\phpseclib3\\Crypt\\RSA');
    }
    /**
     * @return void
     */
    private function checkSimpleJwt()
    {
        ...
        if (!\class_exists(\ShopMagicGoogleSheetsVendor\SimpleJWT\JWT::class)) {
    ...
    {
        return \call_user_func_array([\ShopMagicGoogleSheetsVendor\Firebase\JWT\JWT::class, $method], $args);
        // @phpstan-ignore-line
    }
    /**
     * Provide a hook to mock calls to the JWT static methods.
    ...
    {
        return \call_user_func_array([\ShopMagicGoogleSheetsVendor\SimpleJWT\JWT::class, 'decode'], $args);
    }
    /**
