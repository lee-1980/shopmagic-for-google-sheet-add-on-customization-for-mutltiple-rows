...
class SimpleMiddleware
{
    ...
    public function __invoke(callable $handler)
    {
        return function (\ShopMagicGoogleSheetsVendor\Psr\Http\Message\RequestInterface $request, array $options) use($handler) {
            ...
            $query = \ShopMagicGoogleSheetsVendor\GuzzleHttp\Psr7\Query::parse($request->getUri()->getQuery());
            $params = \array_merge($query, $this->config);
            $uri = $request->getUri()->withQuery(\ShopMagicGoogleSheetsVendor\GuzzleHttp\Psr7\Query::build($params));
