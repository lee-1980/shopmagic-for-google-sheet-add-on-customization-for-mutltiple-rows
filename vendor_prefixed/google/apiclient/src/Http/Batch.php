...
class Batch
{
    ...
    private $rootUrl;
    private $batchPath;
    public function __construct(\ShopMagicGoogleSheetsVendor\Google\Client $client, $boundary = \false, $rootUrl = null, $batchPath = null)
    {
        $this->client = $client;
    ...
    public function add(\ShopMagicGoogleSheetsVendor\Psr\Http\Message\RequestInterface $request, $key = \false)
    {
        if (\false == $key) {
            $key = \mt_rand();
        }
...
EOF;
        ...
        $request = new \ShopMagicGoogleSheetsVendor\GuzzleHttp\Psr7\Request('POST', $url, $headers, $body);
        $response = $this->client->execute($request);
        return $this->parseResponse($response, $classes);
    }
    public function parseResponse(\ShopMagicGoogleSheetsVendor\Psr\Http\Message\ResponseInterface $response, $classes = [])
    {
        ...
        if (!empty($body)) {
            ...
            foreach ($parts as $i => $part) {
                ...
                if (!empty($part)) {
                    ...
                    $response = new \ShopMagicGoogleSheetsVendor\GuzzleHttp\Psr7\Response((int) $status, $partHeaders, \ShopMagicGoogleSheetsVendor\GuzzleHttp\Psr7\Utils::streamFor($partBody));
                    // Need content id.
                    $key = $headers['content-id'];
                    try {
                        $response = \ShopMagicGoogleSheetsVendor\Google\Http\REST::decodeHttpResponse($response, $requests[$i - 1]);
