 ...
 */
namespace ShopMagicGoogleSheetsVendor\Google\Service;

use ShopMagicGoogleSheetsVendor\Google\Exception as GoogleException;
use ShopMagicGoogleSheetsVendor\Google\Http\MediaFileUpload;
use ShopMagicGoogleSheetsVendor\Google\Model;
use ShopMagicGoogleSheetsVendor\Google\Utils\UriTemplate;
...
class Resource
{
    ...
    public function call($name, $arguments, $expectedClass = null)
    {
        if (!isset($this->methods[$name])) {
            ...
            throw new \ShopMagicGoogleSheetsVendor\Google\Exception("Unknown function: " . "{$this->serviceName}->{$this->resourceName}->{$name}()");
        }
        ...
        if (isset($parameters['postBody'])) {
            if ($parameters['postBody'] instanceof \ShopMagicGoogleSheetsVendor\Google\Model) {
        ...
        foreach ($parameters as $key => $val) {
            if ($key != 'postBody' && !isset($method['parameters'][$key])) {
                ...
                throw new \ShopMagicGoogleSheetsVendor\Google\Exception("({$name}) unknown parameter: '{$key}'");
            }
        }
        foreach ($method['parameters'] as $paramName => $paramSpec) {
            if (isset($paramSpec['required']) && $paramSpec['required'] && !isset($parameters[$paramName])) {
                ...
                throw new \ShopMagicGoogleSheetsVendor\Google\Exception("({$name}) missing required param: '{$paramName}'");
            }
        ...
        $request = new \ShopMagicGoogleSheetsVendor\GuzzleHttp\Psr7\Request($method['httpMethod'], $url, $postBody ? ['content-type' => 'application/json'] : [], $postBody ? \json_encode($postBody) : '');
        // support uploads
        if (isset($parameters['data'])) {
            $mimeType = isset($parameters['mimeType']) ? $parameters['mimeType']['value'] : 'application/octet-stream';
            $data = $parameters['data']['value'];
            $upload = new \ShopMagicGoogleSheetsVendor\Google\Http\MediaFileUpload($this->client, $request, $mimeType, $data);
    ...
    public function createRequestUri($restPath, $params)
    {
        ...
        if (\count($uriTemplateVars)) {
            $uriTemplateParser = new \ShopMagicGoogleSheetsVendor\Google\Utils\UriTemplate();
