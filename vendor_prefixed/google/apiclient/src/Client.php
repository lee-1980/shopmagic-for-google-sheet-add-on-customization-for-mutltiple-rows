 ...
 */
namespace ShopMagicGoogleSheetsVendor\Google;

use BadMethodCallException;
use DomainException;
use ShopMagicGoogleSheetsVendor\Google\AccessToken\Revoke;
use ShopMagicGoogleSheetsVendor\Google\AccessToken\Verify;
use ShopMagicGoogleSheetsVendor\Google\Auth\ApplicationDefaultCredentials;
use ShopMagicGoogleSheetsVendor\Google\Auth\Cache\MemoryCacheItemPool;
use ShopMagicGoogleSheetsVendor\Google\Auth\Credentials\ServiceAccountCredentials;
use ShopMagicGoogleSheetsVendor\Google\Auth\Credentials\UserRefreshCredentials;
...
use ShopMagicGoogleSheetsVendor\Google\Auth\OAuth2;
use ShopMagicGoogleSheetsVendor\Google\AuthHandler\AuthHandlerFactory;
use ShopMagicGoogleSheetsVendor\Google\Http\REST;
use ShopMagicGoogleSheetsVendor\GuzzleHttp\Client as GuzzleClient;
use ShopMagicGoogleSheetsVendor\GuzzleHttp\ClientInterface;
use ShopMagicGoogleSheetsVendor\GuzzleHttp\Ring\Client\StreamHandler;
use InvalidArgumentException;
use LogicException;
use ShopMagicGoogleSheetsVendor\Monolog\Handler\StreamHandler as MonologStreamHandler;
use ShopMagicGoogleSheetsVendor\Monolog\Handler\SyslogHandler as MonologSyslogHandler;
...
use ShopMagicGoogleSheetsVendor\Psr\Log\LoggerInterface;
...
class Client
{
    ...
    public function __construct(array $config = [])
    {
        ...
        if (!\is_null($this->config['credentials'])) {
            if ($this->config['credentials'] instanceof \ShopMagicGoogleSheetsVendor\Google\Auth\CredentialsLoader) {
    ...
    public function fetchAccessTokenWithAuthCode($code)
    {
        ...
        $httpHandler = \ShopMagicGoogleSheetsVendor\Google\Auth\HttpHandler\HttpHandlerFactory::build($this->getHttpClient());
     ...
     * @return array access token
     */
    public function fetchAccessTokenWithAssertion(\ShopMagicGoogleSheetsVendor\GuzzleHttp\ClientInterface $authHttp = null)
    {
        ...
        $httpHandler = \ShopMagicGoogleSheetsVendor\Google\Auth\HttpHandler\HttpHandlerFactory::build($authHttp);
     ...
     */
    public function authorize(\ShopMagicGoogleSheetsVendor\GuzzleHttp\ClientInterface $http = null)
    {
    ...
    {
        $tokenRevoker = new \ShopMagicGoogleSheetsVendor\Google\AccessToken\Revoke($this->getHttpClient());
    ...
    {
        $tokenVerifier = new \ShopMagicGoogleSheetsVendor\Google\AccessToken\Verify($this->getHttpClient(), $this->getCache(), $this->config['jwt']);
        if (null === $idToken) {
     ...
     */
    public function execute(\ShopMagicGoogleSheetsVendor\Psr\Http\Message\RequestInterface $request, $expectedClass = null)
    {
        ...
        $http = $this->authorize();
        return \ShopMagicGoogleSheetsVendor\Google\Http\REST::execute($http, $request, $expectedClass, $this->config['retry'], $this->config['retry_map']);
    }
    /**
    ...
    {
        $auth = new \ShopMagicGoogleSheetsVendor\Google\Auth\OAuth2(['clientId' => $this->getClientId(), 'clientSecret' => $this->getClientSecret(), 'authorizationUri' => self::OAUTH2_AUTH_URL, 'tokenCredentialUri' => self::OAUTH2_TOKEN_URI, 'redirectUri' => $this->getRedirectUri(), 'issuer' => $this->config['client_id'], 'signingKey' => $this->config['signing_key'], 'signingAlgorithm' => $this->config['signing_algorithm']]);
        return $auth;
    }
    /**
     ...
     */
    public function setCache(\ShopMagicGoogleSheetsVendor\Psr\Cache\CacheItemPoolInterface $cache)
    {
        $this->cache = $cache;
    }
    /**
    ...
    {
        $logger = new \ShopMagicGoogleSheetsVendor\Monolog\Logger('google-api-php-client');
        if ($this->isAppEngine()) {
            $handler = new \ShopMagicGoogleSheetsVendor\Monolog\Handler\SyslogHandler('app', \LOG_USER, \ShopMagicGoogleSheetsVendor\Monolog\Logger::NOTICE);
        } else {
            $handler = new \ShopMagicGoogleSheetsVendor\Monolog\Handler\StreamHandler('php://stderr', \ShopMagicGoogleSheetsVendor\Monolog\Logger::NOTICE);
        }
    ...
    {
        return new \ShopMagicGoogleSheetsVendor\Google\Auth\Cache\MemoryCacheItemPool();
    }
    /**
     * Set the Http Client object
     * @param ClientInterface $http
     */
    public function setHttpClient(\ShopMagicGoogleSheetsVendor\GuzzleHttp\ClientInterface $http)
    {
        $this->http = $http;
    }
    /**
    ...
    protected function createDefaultHttpClient()
    {
        ...
        if (\defined('ShopMagicGoogleSheetsVendor\\GuzzleHttp\\ClientInterface::MAJOR_VERSION')) {
            $guzzleVersion = \ShopMagicGoogleSheetsVendor\GuzzleHttp\ClientInterface::MAJOR_VERSION;
        } elseif (\defined('ShopMagicGoogleSheetsVendor\\GuzzleHttp\\ClientInterface::VERSION')) {
            $guzzleVersion = (int) \substr(\ShopMagicGoogleSheetsVendor\GuzzleHttp\ClientInterface::VERSION, 0, 1);
        }
        if (5 === $guzzleVersion) {
            $options = ['base_url' => $this->config['base_path'], 'defaults' => ['exceptions' => \false]];
            if ($this->isAppEngine()) {
                if (\class_exists(\ShopMagicGoogleSheetsVendor\GuzzleHttp\Ring\Client\StreamHandler::class)) {
                    // set StreamHandler on AppEngine by default
                    $options['handler'] = new \ShopMagicGoogleSheetsVendor\GuzzleHttp\Ring\Client\StreamHandler();
        ...
        return new \ShopMagicGoogleSheetsVendor\GuzzleHttp\Client($options);
    }
    /**
    ...
    private function createApplicationDefaultCredentials()
    {
        ...
        if ($signingKey) {
            ...
            $credentials = \ShopMagicGoogleSheetsVendor\Google\Auth\CredentialsLoader::makeCredentials($scopes, $serviceAccountCredentials);
        } else {
            ...
            $credentials = \ShopMagicGoogleSheetsVendor\Google\Auth\ApplicationDefaultCredentials::getCredentials($scopes, null, $sub ? null : $this->config['cache_config'], $sub ? null : $this->getCache(), $this->config['quota_project']);
        }
        // for service account domain-wide authority (impersonating a user)
        // @see https://developers.google.com/identity/protocols/OAuth2ServiceAccount
        if ($sub) {
            if (!$credentials instanceof \ShopMagicGoogleSheetsVendor\Google\Auth\Credentials\ServiceAccountCredentials) {
        ...
        if (!$credentials instanceof \ShopMagicGoogleSheetsVendor\Google\Auth\FetchAuthTokenCache) {
            $credentials = new \ShopMagicGoogleSheetsVendor\Google\Auth\FetchAuthTokenCache($credentials, $this->config['cache_config'], $this->getCache());
        }
        return $credentials;
    }
    protected function getAuthHandler()
    {
        ...
        return \ShopMagicGoogleSheetsVendor\Google\AuthHandler\AuthHandlerFactory::build($this->getCache(), $this->config['cache_config']);
    }
    private function createUserRefreshCredentials($scope, $refreshToken)
    {
        $creds = \array_filter(['client_id' => $this->getClientId(), 'client_secret' => $this->getClientSecret(), 'refresh_token' => $refreshToken]);
        return new \ShopMagicGoogleSheetsVendor\Google\Auth\Credentials\UserRefreshCredentials($scope, $creds);
    }
}
