 ...
 * @final
 */
class MessageFormatter implements \ShopMagicGoogleSheetsVendor\GuzzleHttp\MessageFormatterInterface
{
    /**
    ...
    public function format(\ShopMagicGoogleSheetsVendor\Psr\Http\Message\RequestInterface $request, ?\ShopMagicGoogleSheetsVendor\Psr\Http\Message\ResponseInterface $response = null, ?\Throwable $error = null) : string
    {
        ...
        return \preg_replace_callback('/{\\s*([A-Za-z_\\-\\.0-9]+)\\s*}/', function (array $matches) use($request, $response, $error, &$cache) {
            ...
            switch ($matches[1]) {
                case 'request':
                    $result = \ShopMagicGoogleSheetsVendor\GuzzleHttp\Psr7\Message::toString($request);
                    break;
                case 'response':
                    $result = $response ? \ShopMagicGoogleSheetsVendor\GuzzleHttp\Psr7\Message::toString($response) : '';
                    break;
                case 'req_headers':
                ...
                case 'req_body':
                    ...
                    break;
                case 'res_body':
                    if (!$response instanceof \ShopMagicGoogleSheetsVendor\Psr\Http\Message\ResponseInterface) {
                        $result = 'NULL';
                        break;
                    }
     ...
     */
    private function headers(\ShopMagicGoogleSheetsVendor\Psr\Http\Message\MessageInterface $message) : string
    {
        $result = '';
