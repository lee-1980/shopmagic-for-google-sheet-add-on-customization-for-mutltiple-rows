...
use DateTimeZone;
use ShopMagicGoogleSheetsVendor\Monolog\Handler\HandlerInterface;
use ShopMagicGoogleSheetsVendor\Psr\Log\LoggerInterface;
use ShopMagicGoogleSheetsVendor\Psr\Log\InvalidArgumentException;
 ...
 */
class Logger implements \ShopMagicGoogleSheetsVendor\Psr\Log\LoggerInterface, \ShopMagicGoogleSheetsVendor\Monolog\ResettableInterface
{
    /**
     ...
     */
    public function pushHandler(\ShopMagicGoogleSheetsVendor\Monolog\Handler\HandlerInterface $handler) : self
    {
     ...
     */
    public function popHandler() : \ShopMagicGoogleSheetsVendor\Monolog\Handler\HandlerInterface
    {
        if (!$this->handlers) {
     ...
     */
    public function addRecord(int $level, string $message, array $context = [], \ShopMagicGoogleSheetsVendor\Monolog\DateTimeImmutable $datetime = null) : bool
    {
        ...
        try {
            ...
            foreach ($this->handlers as $handler) {
                if (null === $record) {
                    ...
                    $record = ['message' => $message, 'context' => $context, 'level' => $level, 'level_name' => $levelName, 'channel' => $this->name, 'datetime' => $datetime ?? new \ShopMagicGoogleSheetsVendor\Monolog\DateTimeImmutable($this->microsecondTimestamps, $this->timezone), 'extra' => []];
                    try {
    ...
    {
        foreach ($this->handlers as $handler) {
            if ($handler instanceof \ShopMagicGoogleSheetsVendor\Monolog\ResettableInterface) {
                $handler->reset();
            }
        }
        foreach ($this->processors as $processor) {
            if ($processor instanceof \ShopMagicGoogleSheetsVendor\Monolog\ResettableInterface) {
                $processor->reset();
            }
        }
    }
    ...
    public static function getLevelName(int $level) : string
    {
        if (!isset(static::$levels[$level])) {
            throw new \ShopMagicGoogleSheetsVendor\Psr\Log\InvalidArgumentException('Level "' . $level . '" is not defined, use one of: ' . \implode(', ', \array_keys(static::$levels)));
        }
    ...
    public static function toMonologLevel($level) : int
    {
        if (\is_string($level)) {
            ...
            throw new \ShopMagicGoogleSheetsVendor\Psr\Log\InvalidArgumentException('Level "' . $level . '" is not defined, use one of: ' . \implode(', ', \array_keys(static::$levels) + static::$levels));
        }
        if (!\is_int($level)) {
            throw new \ShopMagicGoogleSheetsVendor\Psr\Log\InvalidArgumentException('Level "' . \var_export($level, \true) . '" is not defined, use one of: ' . \implode(', ', \array_keys(static::$levels) + static::$levels));
        }
        return $level;
    }
    /**
