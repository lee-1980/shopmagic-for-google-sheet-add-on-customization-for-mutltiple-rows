...
use ShopMagicGoogleSheetsVendor\Monolog\Utils;
use ShopMagicGoogleSheetsVendor\PhpConsole\Connector;
use ShopMagicGoogleSheetsVendor\PhpConsole\Handler as VendorPhpConsoleHandler;
use ShopMagicGoogleSheetsVendor\PhpConsole\Helper;
/**
 ...
 */
class PHPConsoleHandler extends \ShopMagicGoogleSheetsVendor\Monolog\Handler\AbstractProcessingHandler
{
     ...
     */
    public function __construct(array $options = [], ?\ShopMagicGoogleSheetsVendor\PhpConsole\Connector $connector = null, $level = \ShopMagicGoogleSheetsVendor\Monolog\Logger::DEBUG, bool $bubble = \true)
    {
        if (!\class_exists('ShopMagicGoogleSheetsVendor\\PhpConsole\\Connector')) {
    ...
    private function initConnector(?\ShopMagicGoogleSheetsVendor\PhpConsole\Connector $connector = null) : \ShopMagicGoogleSheetsVendor\PhpConsole\Connector
    {
        if (!$connector) {
            if ($this->options['dataStorage']) {
                \ShopMagicGoogleSheetsVendor\PhpConsole\Connector::setPostponeStorage($this->options['dataStorage']);
            }
            $connector = \ShopMagicGoogleSheetsVendor\PhpConsole\Connector::getInstance();
        }
        if ($this->options['registerHelper'] && !\ShopMagicGoogleSheetsVendor\PhpConsole\Helper::isRegistered()) {
            \ShopMagicGoogleSheetsVendor\PhpConsole\Helper::register();
        }
        if ($this->options['enabled'] && $connector->isActiveClient()) {
            if ($this->options['useOwnErrorsHandler'] || $this->options['useOwnExceptionsHandler']) {
                $handler = \ShopMagicGoogleSheetsVendor\PhpConsole\Handler::getInstance();
    ...
    public function getConnector() : \ShopMagicGoogleSheetsVendor\PhpConsole\Connector
    {
    ...
    {
        if ($record['level'] < \ShopMagicGoogleSheetsVendor\Monolog\Logger::NOTICE) {
    ...
    private function handleDebugRecord(array $record) : void
    {
        ...
        if ($record['context']) {
            $message .= ' ' . \ShopMagicGoogleSheetsVendor\Monolog\Utils::jsonEncode($this->connector->getDumper()->dump(\array_filter($record['context'])), null, \true);
        }
    ...
    {
        return new \ShopMagicGoogleSheetsVendor\Monolog\Formatter\LineFormatter('%message%');
    }
}
