 ...
 */
class BrowserConsoleHandler extends \ShopMagicGoogleSheetsVendor\Monolog\Handler\AbstractProcessingHandler
{
    /** @var bool */
    ...
    {
        return new \ShopMagicGoogleSheetsVendor\Monolog\Formatter\LineFormatter('[[%channel%]]{macro: autolabel} [[%level_name%]]{font-weight: bold} %message%');
    }
    /**
     * {@inheritDoc}
     */
    ...
    {
        return [\ShopMagicGoogleSheetsVendor\Monolog\Logger::DEBUG => 'debug', \ShopMagicGoogleSheetsVendor\Monolog\Logger::INFO => 'info', \ShopMagicGoogleSheetsVendor\Monolog\Logger::NOTICE => 'info', \ShopMagicGoogleSheetsVendor\Monolog\Logger::WARNING => 'warn', \ShopMagicGoogleSheetsVendor\Monolog\Logger::ERROR => 'error', \ShopMagicGoogleSheetsVendor\Monolog\Logger::CRITICAL => 'error', \ShopMagicGoogleSheetsVendor\Monolog\Logger::ALERT => 'error', \ShopMagicGoogleSheetsVendor\Monolog\Logger::EMERGENCY => 'error'][$level] ?? 'log';
    }
    /**
     * @return string[]
     */
    private static function handleStyles(string $formatted) : array
    {
        ...
        foreach (\array_reverse($matches) as $match) {
            ...
            $pos = $match[0][1];
            $format = \ShopMagicGoogleSheetsVendor\Monolog\Utils::substr($format, 0, $pos) . '%c' . $match[1][0] . '%c' . \ShopMagicGoogleSheetsVendor\Monolog\Utils::substr($format, $pos + \strlen($match[0][0]));
        }
    ...
    private static function handleCustomStyles(string $style, string $string) : string
    {
        ...
        if (null === $style) {
            ...
            throw new \RuntimeException('Failed to run preg_replace_callback: ' . $pcreErrorCode . ' / ' . \ShopMagicGoogleSheetsVendor\Monolog\Utils::pcreLastErrorMessage($pcreErrorCode));
        }
        return $style;
    }
    /**
