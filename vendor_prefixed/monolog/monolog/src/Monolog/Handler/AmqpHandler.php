...
use ShopMagicGoogleSheetsVendor\Monolog\Formatter\JsonFormatter;
use ShopMagicGoogleSheetsVendor\PhpAmqpLib\Message\AMQPMessage;
use ShopMagicGoogleSheetsVendor\PhpAmqpLib\Channel\AMQPChannel;
use AMQPExchange;
/**
 * @phpstan-import-type Record from \Monolog\Logger
 */
class AmqpHandler extends \ShopMagicGoogleSheetsVendor\Monolog\Handler\AbstractProcessingHandler
{
    /**
     ...
     */
    public function __construct($exchange, ?string $exchangeName = null, $level = \ShopMagicGoogleSheetsVendor\Monolog\Logger::DEBUG, bool $bubble = \true)
    {
        if ($exchange instanceof \ShopMagicGoogleSheetsVendor\PhpAmqpLib\Channel\AMQPChannel) {
    ...
    private function createAmqpMessage(string $data) : \ShopMagicGoogleSheetsVendor\PhpAmqpLib\Message\AMQPMessage
    {
        $attributes = ['delivery_mode' => 2, 'content_type' => 'application/json'];
        if ($this->extraAttributes) {
            $attributes = \array_merge($attributes, $this->extraAttributes);
        }
        return new \ShopMagicGoogleSheetsVendor\PhpAmqpLib\Message\AMQPMessage($data, $attributes);
    }
    /**
     * {@inheritDoc}
     */
    ...
    {
        return new \ShopMagicGoogleSheetsVendor\Monolog\Formatter\JsonFormatter(\ShopMagicGoogleSheetsVendor\Monolog\Formatter\JsonFormatter::BATCH_MODE_JSON, \false);
    }
}
