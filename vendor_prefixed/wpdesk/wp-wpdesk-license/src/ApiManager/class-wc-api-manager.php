...
use ShopMagicGoogleSheetsVendor\WPDesk\Notice\AjaxHandler;
use ShopMagicGoogleSheetsVendor\WPDesk\Notice\PermanentDismissibleNotice;
if (!\defined('ABSPATH')) {
    exit;
}
if (!\class_exists('ShopMagicGoogleSheetsVendor\\WPDesk_API_Manager_With_Update_Flag')) {
    class WPDesk_API_Manager_With_Update_Flag
    {
        ...
        {
            $password_management = new \ShopMagicGoogleSheetsVendor\WPDesk_API_Password_Management();
        ...
        public function key()
        {
            if ($this->key_insance == null) {
                $this->key_insance = new \ShopMagicGoogleSheetsVendor\WPDesk_API_KEY($this);
            }
        ...
        public function update_check($upgrade_url, $plugin_name, $product_id, $api_key, $activation_email, $renew_license_url, $instance, $domain, $software_version, $plugin_or_theme, $text_domain, $extra = '')
        {
            if ($this->update_check_insance == null) {
                $this->update_check_insance = new \ShopMagicGoogleSheetsVendor\WPDesk_Update_API_Check($this, $upgrade_url, $plugin_name, $product_id, $api_key, $activation_email, $renew_license_url, $instance, $domain, $software_version, $plugin_or_theme, $text_domain, $extra);
            }
        ...
        public function check_external_blocking()
        {
            ...
            if (\defined('ShopMagicGoogleSheetsVendor\\WP_HTTP_BLOCK_EXTERNAL') && \ShopMagicGoogleSheetsVendor\WP_HTTP_BLOCK_EXTERNAL === \true) {
                // check if our API endpoint is in the allowed hosts
                $host = \parse_url($this->upgrade_url, \PHP_URL_HOST);
                if (!\defined('ShopMagicGoogleSheetsVendor\\WP_ACCESSIBLE_HOSTS') || \stristr(\ShopMagicGoogleSheetsVendor\WP_ACCESSIBLE_HOSTS, $host) === \false) {
                    ?>
                    <div class="error">
                        <p><?php 
        ...
        {
            $parser = new \ShopMagicGoogleSheetsVendor\WPDesk\License\Changelog\Parser($changelog);
            $parser->parse();
            $parsed_changelog = $parser->get_parsed_changelog()->getIterator();
            $changes = new \ShopMagicGoogleSheetsVendor\WPDesk\License\Changelog\Filter\ByVersion($parsed_changelog, $plugin_version);
            if (\iterator_count($changes) > 0) {
                $changelog = new \ShopMagicGoogleSheetsVendor\WPDesk\License\Changelog\Formatter($changes);
