...
use ShopMagicGoogleSheetsVendor\WPDesk\License\PluginRegistratorInterface;
...
class PluginRegistrator implements \ShopMagicGoogleSheetsVendor\WPDesk\License\PluginRegistratorInterface
{
    ...
    {
        return (new \ShopMagicGoogleSheetsVendor\WPDesk\License\LicenseServer\PluginLicense($this->plugin_info))->is_active();
    }
    public function initialize_license_manager()
    {
        (new \ShopMagicGoogleSheetsVendor\WPDesk\License\LicenseServer\PluginUpgrade($this->plugin_info, $this->server, self::get_token()))->hooks();
        (new \ShopMagicGoogleSheetsVendor\WPDesk\License\LicenseServer\PluginExternalBlocking($this->plugin_info, $this->server, self::get_token()))->hooks();
        (new \ShopMagicGoogleSheetsVendor\WPDesk\License\LicenseServer\PluginViewVersionInfo($this->plugin_info, $this->server))->hooks();
    }
}
