...
if (!\class_exists('ShopMagicGoogleSheetsVendor\\WPDesk_Tracker_Sender_Logged')) {
    class WPDesk_Tracker_Sender_Logged implements \WPDesk_Tracker_Sender
    {
        ...
        public function send_payload(array $payload)
        {
            if (\class_exists('ShopMagicGoogleSheetsVendor\\WPDesk_Logger_Factory')) {
                \ShopMagicGoogleSheetsVendor\WPDesk_Logger_Factory::log_message("Sender payload: " . \json_encode($payload), self::LOGGER_SOURCE, \ShopMagicGoogleSheetsVendor\WPDesk_Logger::DEBUG);
                try {
                    $response = $this->sender->send_payload($payload);
                    \ShopMagicGoogleSheetsVendor\WPDesk_Logger_Factory::log_message('Sender response: ' . \json_encode($response), self::LOGGER_SOURCE, \ShopMagicGoogleSheetsVendor\WPDesk_Logger::DEBUG);
                    return $response;
                } catch (\ShopMagicGoogleSheetsVendor\WPDesk_Tracker_Sender_Exception_WpError $exception) {
                    \ShopMagicGoogleSheetsVendor\WPDesk_Logger_Factory::log_exception($exception);
                    throw $exception;
                }
            } else {
